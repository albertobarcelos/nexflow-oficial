-- =====================================================
-- ADICIONAR SNAPSHOT DE CAMPOS AO MUDAR DE ETAPA
-- =====================================================
-- Modifica o trigger track_card_stage_change() para capturar
-- um snapshot dos campos preenchidos na etapa anterior e
-- armazenar no details do evento stage_change
-- =====================================================
  ...
  IF OLD.step_id IS NOT NULL AND OLD.field_values IS NOT NULL THEN
    ...
    LOOP
      ...
      IF field_value IS NOT NULL AND field_value != 'null'::jsonb AND field_value != '""'::jsonb THEN
        ...
        -- Buscar quando o campo foi preenchido (último evento field_update para este campo nesta etapa)
        -- Se não encontrar, usar updated_at do card como fallback
        SELECT COALESCE(
...
COMMENT ON FUNCTION public.track_card_stage_change() IS 'Cria evento stage_change no histórico quando step_id muda, incluindo snapshot dos campos preenchidos na etapa anterior';

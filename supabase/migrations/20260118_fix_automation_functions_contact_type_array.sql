-- Migration: Corrigir funções de automação para usar sintaxe correta de array
-- Data: 2026-01-18
-- Problema: As funções ainda comparam contact_type como string quando é um array TEXT[]

-- 1. Corrigir função de automação de parceiros
...
-- 2. Corrigir função de automação de clientes
...
'Automação: Quando um contato com contact_type contendo "parceiro" é criado/atualizado, cria ou atualiza card no flow "Parceiros". Usa sintaxe de array: ''parceiro'' = ANY(contact_type).';

COMMENT ON FUNCTION public.handle_client_contact_automation IS 
'Automação: Quando um contato com contact_type contendo "cliente" é criado, cria automaticamente um card no flow "Vendas" na primeira etapa. Usa sintaxe de array: ''cliente'' = ANY(contact_type).';

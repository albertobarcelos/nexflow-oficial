-- AIDEV-NOTE: Migração para adicionar função de busca flexível
...
BEGIN
  ...
  WITH search_results AS (
    ...
    AND (
      ...
      regexp_replace(COALESCE(wc.cnpj, ''), '[^0-9]', '', 'g') ILIKE '%' || clean_search_term || '%' OR
      wc.whatsapp ILIKE '%' || p_search_term || '%' OR
      regexp_replace(COALESCE(wc.whatsapp, ''), '[^0-9]', '', 'g') ILIKE '%' || clean_search_term || '%' OR
      wc.telefone ILIKE '%' || p_search_term || '%' OR
      regexp_replace(COALESCE(wc.telefone, ''), '[^0-9]', '', 'g') ILIKE '%' || clean_search_term || '%' OR
      wc.celular ILIKE '%' || p_search_term || '%' OR
      regexp_replace(COALESCE(wc.celular, ''), '[^0-9]', '', 'g') ILIKE '%' || clean_search_term || '%' OR
      wc.phone ILIKE '%' || p_search_term || '%' OR
      regexp_replace(COALESCE(wc.phone, ''), '[^0-9]', '', 'g') ILIKE '%' || clean_search_term || '%' OR
      wc.cep ILIKE '%' || p_search_term || '%' OR
      regexp_replace(COALESCE(wc.cep, ''), '[^0-9]', '', 'g') ILIKE '%' || clean_search_term || '%' OR
      wc.numero ILIKE '%' || p_search_term || '%' OR
      regexp_replace(COALESCE(wc.numero, ''), '[^0-9]', '', 'g') ILIKE '%' || clean_search_term || '%'
    )

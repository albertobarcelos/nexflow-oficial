-- =====================================================
-- SISTEMA DE COMISSÕES POR TIME (BASEADO EM CARDS)
-- =====================================================
...
--
-- BASEADO EM: nexflow.cards (não web_deals)
-- Cards são o centro da operação do closer
-- =====================================================
...
-- NOTA: nexflow.cards já tem assigned_team_id, não precisa adicionar!
...
CREATE INDEX IF NOT EXISTS idx_team_member_levels_level ON core_team_member_levels(team_level_id);
CREATE INDEX IF NOT EXISTS idx_team_member_levels_active ON core_team_member_levels(team_member_id, effective_to) WHERE effective_to IS NULL;
...
-- =====================================================
-- 4. TABELA: COMISSÕES POR TIME E ITEM
-- =====================================================

CREATE TABLE IF NOT EXISTS core_team_commissions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  team_id UUID NOT NULL REFERENCES core_teams(id) ON DELETE CASCADE,
  item_id UUID REFERENCES web_items(id) ON DELETE SET NULL, -- NULL = todos os itens
  item_code VARCHAR(100), -- Código do item (ex: "XPTO") para busca rápida
  ...
  CONSTRAINT unique_team_item_commission UNIQUE(team_id, COALESCE(item_id::text, ''), COALESCE(item_code, ''))
);

CREATE INDEX IF NOT EXISTS idx_team_commissions_team ON core_team_commissions(team_id);
CREATE INDEX IF NOT EXISTS idx_team_commissions_item ON core_team_commissions(item_id);
CREATE INDEX IF NOT EXISTS idx_team_commissions_code ON core_team_commissions(item_code);
...
-- =====================================================
-- 5. TABELA: ITENS DE CARD (PRODUTOS/SERVIÇOS VENDIDOS)
-- =====================================================
-- Schema: nexflow (porque está vinculado a nexflow.cards)
...
CREATE TABLE IF NOT EXISTS nexflow.card_items (
  ...
  item_name VARCHAR(255) NOT NULL, -- Nome do item (snapshot no momento da venda)
...
CREATE INDEX IF NOT EXISTS idx_card_items_card ON nexflow.card_items(card_id);
CREATE INDEX IF NOT EXISTS idx_card_items_item ON nexflow.card_items(item_id);
CREATE INDEX IF NOT EXISTS idx_card_items_code ON nexflow.card_items(item_code);

-- Trigger para atualizar updated_at
CREATE TRIGGER trigger_update_card_items_updated_at
  BEFORE UPDATE ON nexflow.card_items
  FOR EACH ROW
  EXECUTE FUNCTION update_team_levels_updated_at();

-- Trigger para calcular total_price automaticamente
CREATE OR REPLACE FUNCTION calculate_card_item_total()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_calculate_card_item_total
  BEFORE INSERT OR UPDATE ON nexflow.card_items
  FOR EACH ROW
...
-- =====================================================
-- NOTA: Esta tabela será ajustada na migração de pagamentos
-- para incluir campos payment_id, payment_amount, payment_date
...
CREATE TABLE IF NOT EXISTS core_commission_calculations (
  ...
  payment_id UUID, -- Será referenciado na migração de pagamentos
...
CREATE INDEX IF NOT EXISTS idx_commission_calc_card ON core_commission_calculations(card_id);
...
CREATE INDEX IF NOT EXISTS idx_commission_calc_payment ON core_commission_calculations(payment_id) WHERE payment_id IS NOT NULL;
...
-- Função para buscar comissão do time para um item
...
BEGIN
  ...
  WHERE tc.team_id = p_team_id
    ...
    AND (
      (p_item_id IS NOT NULL AND tc.item_id = p_item_id)
      OR (p_item_code IS NOT NULL AND tc.item_code = p_item_code)
      OR (tc.item_id IS NULL AND tc.item_code IS NULL) -- Comissão padrão
    )
  ORDER BY 
    CASE WHEN tc.item_id IS NOT NULL THEN 1 ELSE 2 END, -- Prioridade: item específico
    CASE WHEN tc.item_code IS NOT NULL THEN 1 ELSE 2 END
  LIMIT 1;
END;
$$ LANGUAGE plpgsql;
...
ALTER TABLE nexflow.card_items ENABLE ROW LEVEL SECURITY;
...
-- Políticas para nexflow.card_items
CREATE POLICY "Users can view card items of their client"
  ON nexflow.card_items FOR SELECT
  USING (
    client_id IN (
      SELECT client_id FROM core_client_users WHERE id = auth.uid()
    )
...
CREATE POLICY "Users can manage card items of their client"
  ON nexflow.card_items FOR ALL
  USING (
    client_id IN (
      SELECT client_id FROM core_client_users WHERE id = auth.uid()
    )
...
COMMENT ON TABLE core_team_commissions IS 'Configuração de comissões por time e item (produto/serviço)';
COMMENT ON TABLE nexflow.card_items IS 'Itens (produtos/serviços) vendidos em um card';
COMMENT ON TABLE core_commission_calculations IS 'Cálculos de comissão realizados para cards completados quando pagamento é recebido';
...
COMMENT ON COLUMN core_team_commissions.item_code IS 'Código do item (ex: "XPTO") para busca rápida';
COMMENT ON COLUMN core_team_commissions.commission_type IS 'Tipo: "percentage" (percentual) ou "fixed" (valor fixo)';
COMMENT ON COLUMN nexflow.card_items.item_code IS 'Código do item no momento da venda (snapshot)';
COMMENT ON COLUMN nexflow.card_items.installment_number IS 'Número da parcela (NULL se não parcelado)';
COMMENT ON COLUMN nexflow.card_items.total_installments IS 'Total de parcelas (NULL se não parcelado)';
COMMENT ON COLUMN core_commission_calculations.card_id IS 'Card (nexflow.cards) que gerou a comissão';
COMMENT ON COLUMN core_commission_calculations.payment_id IS 'Pagamento (web_payments) que gerou a comissão - comissão só é calculada quando pagamento é recebido';

...
serve(async (req) => {
  ...
  try {
    ...
    const { opportunity_id, client_id, automation_id }: AutoCreateRequest = await req.json()

    // Validação dos campos obrigatórios
    if (!opportunity_id || !client_id) {
      return new Response(
        JSON.stringify({ error: 'Campos obrigatórios faltando: opportunity_id, client_id' }),
        { 
          status: 400,
    ...
    // Buscar a oportunidade (no schema nexflow)
    ...
    if (oppError || !opportunity) {
      return new Response(
        JSON.stringify({ error: 'Oportunidade não encontrada ou acesso negado' }),
        { 
          status: 404,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    for (const automation of automations) {
      try {
        ...
        const cardTitle = (opportunity as any).name || (opportunity as any).client_name || 'Nova Oportunidade'

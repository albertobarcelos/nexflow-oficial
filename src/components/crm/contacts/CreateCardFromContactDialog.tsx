...
import { useCreateCardFromContact } from "@/hooks/useCreateCardFromContact";
...
interface CreateCardFromContactDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  contact: Contact | null;
}

export function CreateCardFromContactDialog({
  open,
  onOpenChange,
  contact,
}: CreateCardFromContactDialogProps) {
  ...
  const { mutateAsync: createCard, isPending: isCreating } =
    useCreateCardFromContact();

  const handleCreate = async () => {
    if (!contact || !selectedFlowId || !selectedStepId) {
      return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Criar Card a partir de Contato</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={
                contact?.client_name || "Título do card"
              }
            />

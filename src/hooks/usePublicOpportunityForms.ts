...
export interface PublicOpportunityForm {
  id: string;
  client_id: string;
  title: string;
  description?: string;
...
export function usePublicOpportunityForms() {
  const queryClient = useQueryClient();

  const { data: forms, isLoading, error } = useQuery({
    queryKey: ["public-opportunity-forms"],
    queryFn: async () => {
      ...
      return data as PublicOpportunityForm[];
    },
  });
  ...
  const createForm = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["public-opportunity-forms"] });
  ...
  const updateForm = useMutation({
    mutationFn: async ({ id, ...updates }: Partial<PublicOpportunityForm> & { id: string }) => {
      const { data, error } = await supabase
        .from("public_opportunity_forms")
        .update(updates)
        .eq("id", id)

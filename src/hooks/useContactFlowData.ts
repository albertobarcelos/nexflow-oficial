...
/**
 * Dados do gráfico de fluxo de contatos por período (multi-tenant seguro).
...
export function useContactFlowData(period: PeriodFilter = "today") {
  ...
  const { data, isLoading } = useSecureClientQuery<ChartDataPoint[]>({
    queryKey: ["contact-flow-data", period],
    queryFn: async (client, clientId): Promise<ChartDataPoint[]> => {
